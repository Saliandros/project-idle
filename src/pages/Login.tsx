...
export function Login({ onLogin }: LoginProps) {
	...
	const imageAspectRatio = useMemo(() => {
		const backgroundImage = theme.backgrounds.WorldMapImage as number | { width?: number; height?: number };

		if (typeof backgroundImage === 'object' && backgroundImage.width && backgroundImage.height) {
			return backgroundImage.width / backgroundImage.height;
		}

		if (Platform.OS !== 'web' && typeof Image.resolveAssetSource === 'function') {
			const resolvedImage = Image.resolveAssetSource(theme.backgrounds.WorldMapImage);
	...
	const coveredImageWidth = useMemo(() => {
		...
		return Math.max(viewport.width, viewport.height * imageAspectRatio) * backgroundScale;
	...
	return (
		...
		>
			...
			<View style={[styles.overlay, { backgroundColor: theme.colors.background }]}>
				{view === 'login' ? (
					<LoginCard onRegister={() => setView('register')} onSubmit={onLogin} />
				) : (
